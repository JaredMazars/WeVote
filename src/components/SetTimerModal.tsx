...
const SetTimerModal: React.FC<SetTimerModalProps> = ({ isOpen, onClose, sessionId }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (sessionId) {
        // Load schedule from real session via API
        api.getSessions().then((res) => {
          if (res.success && Array.isArray(res.data)) {
            const session = (res.data as any[]).find((s: any) => s.SessionID === sessionId);
            if (session?.ScheduledStartTime) {
              const dt = new Date(session.ScheduledStartTime);
            ...
            if (session?.ScheduledEndTime) {
              const dt = new Date(session.ScheduledEndTime);
              setEndTime(`${String(dt.getHours()).padStart(2, '0')}:${String(dt.getMinutes()).padStart(2, '0')}`);
            }
          }
        }).catch(() => {
          setStartDate(new Date().toISOString().split('T')[0]);
  ...
  const validateAndSave = async () => {
    ...
    // Build end datetime on same date (or next day if end < start)
    ...
    try {
      if (sessionId) {
        ...
        if (!response.success) {
          setError(response.message || 'Failed to save session schedule');
          setIsSaving(false);
          return;
        }
      } else {
        // Fall back to localStorage when no sessionId
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="flex space-x-4 mt-8">
              ...
              >
                {isSaving ? 'Saving...' : success ? 'Saved!' : 'Save Timer'}
              </button>
            </div>
          </motion.div>
        </div>
